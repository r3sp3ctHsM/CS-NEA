...
namespace MidiPlayerTK
{
    ...
    public class fluid_voice
    {
        ...
        /* pan */
        public float pan;
        public float amp_left;
        public float amp_right;
        ...
        static int[] list_of_weakgenerators_to_initialize =
     {
        ...
        public void fluid_voice_init(fluid_channel pchannel, int pkey, int pvel/*, double gain*/)
        {
            key = pkey;
            vel = pvel;
            channel = pchannel;
        ...
        public void fluid_voice_start()
        {
            ...
            Volume = synth.MPTK_Volume * (float)(fluid_conv.fluid_atten2amp(attenuation) * fluid_conv.fluid_cb2amp(960.0f * (1f - volenv_val)));
            ...
            TimeAtEnd = TimeAtStart + DurationTick;
        ...
        void fluid_voice_calculate_runtime_synthesis_parameters()
        {
            ...
            foreach (HiMod m in mods)
            {
                ...
                gens[m.Dest].Mod += m.fluid_mod_get_value(channel, key, vel);
            }
        ...
        float fluid_voice_get_lower_boundary_for_attenuation()
        {
            ...
            foreach (HiMod m in mods)
            {
                    ...
                    && ((m.Flags1 & (byte)fluid_mod_flags.FLUID_MOD_CC) > 0 || (m.Flags2 & (byte)fluid_mod_flags.FLUID_MOD_CC) > 0))
                {
                    ...
                    float current_val = m.fluid_mod_get_value(channel, key, vel);
            ...
            switch (igen)
            {
                ...
                case (int)fluid_gen_type.GEN_PAN:
                    ...
                    if (synth.MPTK_CorePlayer)
                    {
                        ...
                        {
                            amp_left = fluid_conv.fluid_pan(pan, true) * synth.gain;// / 32768.0f;
                            amp_right = fluid_conv.fluid_pan(pan, false) * synth.gain;// / 32768.0f;
                        }
                        else
                        {
                            amp_left = 1f;
                        ...
                        if (synth.VerboseCalcGen)
                            Debug.LogFormat("Calc {0} synth.gain={1:0.00} pan={2:0.00} amp_left={3:0.00} amp_right={4:0.00}",
                                (fluid_gen_type)igen, synth.gain, pan, amp_left, amp_right);
                    }
                ...
                case (int)fluid_gen_type.GEN_REVERBSEND:
                    ...
                    amp_reverb = reverb_send * synth.gain;// / 32768f;
                    if (synth.VerboseCalcGen)
                ...
                case (int)fluid_gen_type.GEN_CHORUSSEND:
                    ...
                    amp_chorus = chorus_send * synth.gain;// / 32768f;
                    if (synth.VerboseCalcGen)
                ...
                case (int)fluid_gen_type.GEN_FILTERQ:
                    ...
                    float q_dB = (gens[igen].Val + gens[igen].Mod /*+ gens[igen].nrpn*/) / 10f;

                    // Range: SF2.01 section 8.1.3 # 8 (convert from cB to dB => /10)
                    q_dB = q_dB < 0f ? 0f : q_dB > 96f ? 96f : q_dB;
            ...
            for (int i = 0; i < mods.Count; i++)
            {
                    ...
                    (((m.Src2 == ctrl) && ((m.Flags2 & (byte)fluid_mod_flags.FLUID_MOD_CC) == 0) && (cc == 0)))))
                {
                    ...
                    for (int j = 0; j < mods.Count; j++)
                    {
                        ...
                        {
                            modval += m1.fluid_mod_get_value(channel, key, vel);
                        }
                    }
                    ...
                    float[] dsp_reverb_buf, float[] dsp_chorus_buf)
        {
            ...
            // A single tick represents one hundred nanoseconds or one ten-millionth of a second.
            // There are 10,000 ticks in a millisecond, or 10 million ticks in a second. 
            if (DurationTick >= 0 && volenv_section != fluid_voice_envelope_index.FLUID_VOICE_ENVRELEASE && System.DateTime.Now.Ticks > TimeAtEnd)
            {
                //if (synth.VerboseEnvVolume) DebugEnveloppe("Over duration");
                fluid_voice_noteoff();
            }
            ...
            if (synth.VerboseVolume)
                Debug.LogFormat("Volume - [{0:4}] TimeFromStart:{1} Delta:{2:F2} section:{3} amp_max:{4:F2} target_amp:{5:F2} amp:{6:F2} reaches_noise:{7:F2}",
                  IdVoice, TicksToMilli(System.DateTime.Now.Ticks - TimeAtStart), TicksToMilliF(DeltaTimeWrite), volenv_section,
            ...
            locfres = fluid_conv.fluid_ct2hz(fres + modlfo_val * modlfo_to_fc + modenv_val * modenv_to_fc);
            ...
            if ((Mathf.Abs(locfres - last_fres) > 0.01f))
            {
                ...
                * bandwidth readjustment'. */
                ...
                float omega = (2f * M_PI * (locfres / 44100f));
        ...
        void fluid_voice_effects(int count, float[] dsp_left_buf, float[] dsp_right_buf, float[] dsp_reverb_buf, float[] dsp_chorus_buf)
        {
            ...
            if (Mathf.Abs(dsp_hist1) < 1e-20) dsp_hist1 = 0f;  /* FIXME JMG - Is this even needed? */
            ...
            */
            ...
            if ((dsp_reverb_buf != null) && (amp_reverb != 0f))
            {
                for (dsp_i = 0; dsp_i < count; dsp_i++)
                    dsp_reverb_buf[dsp_i] += amp_reverb * dsp_buf[dsp_i];
            }

            /* chorus send. Buffer may be NULL. */
            if ((dsp_chorus_buf != null) && (amp_chorus != 0))
            {
                for (dsp_i = 0; dsp_i < count; dsp_i++)
                    dsp_chorus_buf[dsp_i] += amp_chorus * dsp_buf[dsp_i];
            }
            ...
            if (status == fluid_voice_status.FLUID_VOICE_ON || status == fluid_voice_status.FLUID_VOICE_SUSTAINED)
            {
                if (!weakDevice)
                {
                    ...
                    else
                    {
                        ...
                        volenv_section = fluid_voice_envelope_index.FLUID_VOICE_ENVRELEASE;
                        volenv_count = 0;
                        if (synth.VerboseEnvVolume) DebugVolEnv("fluid_voice_noteoff vol.");

                        modenv_section = fluid_voice_envelope_index.FLUID_VOICE_ENVRELEASE;
                        modenv_count = 0;
                        if (synth.VerboseEnvModulation) DebugModEnv("fluid_voice_noteoff mod.");
                    }
                }
                else
                {
        ...
        */
        ...
        {
            if (!weakDevice)
                Debug.LogFormat("VolEnv - [{0:4}] {1} TimeFromStart:{2} Delta:{3:F2} section:{4} volenv_val:{5:0.000} volenv_count:{6} incr:{7:0.0000}",
                   IdVoice, info, TicksToMilli(System.DateTime.Now.Ticks - TimeAtStart), TicksToMilliF(DeltaTimeWrite),
        ...
        {
            if (!weakDevice)
                Debug.LogFormat("ModEnv - [{0:4}] {1} TimeFromStart:{2:0.000} Delta:{3:0.000} section:{4} modenv_val:{5:0.000} modenv_count:{6} incr:{7:0.0000}",
               IdVoice, info, (System.DateTime.Now.Ticks - TimeAtStart) / fluid_voice.Nano100ToMilli, DeltaTimeWrite / Nano100ToMilli,
        ...
        {
            Debug.LogFormat("[{0:4}] {1} TimeFromStart:{2:00000.000} Delta:{3:0.000} modlfo_delay:{4} modlfo_incr:{5:0.000} modlfo_val:{6:0.000} modlfo_to_vol:{7:0.000}",
               IdVoice, info, (System.DateTime.Now.Ticks - TimeAtStart) / fluid_voice.Nano100ToMilli, DeltaTimeWrite / Nano100ToMilli, modlfo_delay, modlfo_incr, modlfo_val, modlfo_to_vol);
        }
        public void DebugVib(string info)
        {
            Debug.LogFormat("[{0:4}] {1} TimeFromStart:{2:00000.000} Delta:{3:0.000} viblfo_delay:{4} viblfo_incr:{5:0.000} viblfo_val:{6:0.000} viblfo_to_pitch:{7:0.000} -. pitch mod:{8:0.000}",
               IdVoice, info, (System.DateTime.Now.Ticks - TimeAtStart) / fluid_voice.Nano100ToMilli, DeltaTimeWrite / Nano100ToMilli, viblfo_delay, viblfo_incr, viblfo_val, viblfo_to_pitch, (float)(1d + viblfo_val * viblfo_to_pitch / 1000d));
        }
    }
}

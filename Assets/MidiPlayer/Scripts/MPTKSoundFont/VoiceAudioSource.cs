...
namespace MidiPlayerTK
{
    public class VoiceAudioSource : MonoBehaviour
    {
...
#endif
            Timing.RunCoroutine(ThreadPlayNote(), Segment.Update);
        ...
        protected IEnumerator<float> ThreadPlayNote()
        {
            ...
            {
                Audiosource.volume = fluidvoice.Volume;
                ...
                while (fluidvoice.status != fluid_voice_status.FLUID_VOICE_OFF)
                {
                    ...
                    // Wait next iteration
                    yield return Timing.WaitForSeconds(0.010f); //0;
                }
            }
            try
            {
        ...
        {
            fluidvoice.NewTimeWrite = System.DateTime.Now.Ticks;
        ...
        {
            fluidvoice.DeltaTimeWrite = System.DateTime.Now.Ticks - fluidvoice.LastTimeWrite;
            fluidvoice.LastTimeWrite = System.DateTime.Now.Ticks;
